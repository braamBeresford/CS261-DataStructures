...
{
	/* FIXME: You will write this function */
	struct Link* newVal = malloc(sizeof(struct Link));
	assert(newVal);
	newVal->value = value;
	newVal->next= NULL;

	queue->tail->next = newVal;
	queue->tail= newVal;
}
...
{
	 /* FIXME: You will write this function */
	 struct Stack* newStk = malloc(sizeof(struct Stack));
	 assert(newStk);

	 newStk->q1 = listQueueCreate();
	 newStk->q2 = listQueueCreate();
	 return newStk;
};
